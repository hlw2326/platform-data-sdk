...
$normalizedUserTotalKeys = ['fan_count', 'follow_count', 'work_count', 'like_count'];
$normalizedFeedKeys = [
    'platform',
    'type',
    'item_id',
...
assert_keys($dyUserInfo['total'], $normalizedUserTotalKeys, 'Dy UserInfo HTML total 字段不一致');
assert_true($dyUserInfo['total']['fan_count'] === 9 && $dyUserInfo['total']['follow_count'] === 8 && $dyUserInfo['total']['work_count'] === 7 && $dyUserInfo['total']['like_count'] === 6, 'Dy UserInfo HTML total 统计字段失败');
...
assert_true($liveInfo->total['fan_count'] === 1 && $liveInfo->total['like_count'] === 4, 'Live total 统计字段失败');
...
assert_true($ksInfo->user_id === 'uid' && $ksInfo->total['fan_count'] === 3, 'Ks toUserInfo 下划线字段失败');
